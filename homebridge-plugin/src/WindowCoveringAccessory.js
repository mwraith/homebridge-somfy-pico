...
// Helper class used to send the commands to the Pico whilst maintaining
...
export default class WindowCoveringAccessory {
     ...
     * is not supported it is either 100 (OPEN) or 0 (CLOSED).
     *
     * @returns {Integer} 100 = OPEN; 0 = CLOSED
     */
     ...
     * so that 100 = OPEN and 0 = CLOSED.
     *
     * @param {Integer} value - The target position (0 or 100)
     */
    async setTargetPosition(value) {
        ...
        if (this.currentState && this.currentState.positionState !== PositionState.STOPPED) {
            ...
            SendCommand.sendCommand(this.api, this.config, 'My', this.log);
        ...
        // Send button press to Pico
        SendCommand.sendCommand(this.api, this.config, button, this.log);
    ...
    /**
     * Advances the mocked current position of the blind by a given percentage.
     * Called on an interval until the blind reaches its target position.
     *
     * @param {Integer} increment - The percentage to advance the current position
     */
    ...
    /**
     * Returns the current position state of the blind.
     *
     * @returns {Characteristic.PositionState}
     */
    ...
    /**
     * Gets the target position for the blind from BlindState.
     *
     * @returns {Integer} 100 = OPEN; 0 = CLOSED
     */
